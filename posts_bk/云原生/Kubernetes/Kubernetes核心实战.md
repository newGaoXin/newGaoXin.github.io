...
```yaml
apiVersion: v1   # 固定格式
kind: Namespace  # 指定是创建名称空间
metadata:
  name: hello    # 指定名称空间 name
```
...
```yaml
apiVersion: apps/v1 # 固定格式
kind: Deployment # 指定操作类型 Deployment
metadata: # 具体表现信息
  labels:
    app: my-dep
  name: my-dep # 指定名称
...
```bash
# 查看指定ploy的yaml信息
kubectl get deploy [deploy名称] -oyaml
...
spec:
  selector:
    app: my-dep	# 指定的deploy名称
  ports:
  - port: 8000	# service的端口
    protocol: TCP
    targetPort: 80 # 目标端口，即pod容器的端口
```



## 1、ClusterIP（只允许在 k8s 集群中访问）
...
kind: Ingress  
metadata:
  name: ingress-host-bar # 规则名称
spec:
  ingressClassName: nginx	# 类型
  rules:
  - host: "hello.atguigu.com"	# 转发的域名
    http:
      paths:
      - pathType: Prefix
        path: "/"	# 路径
        backend:
          service: # 转发到哪个service
            name: hello-server # service 名称
            port:
              number: 8000	# service 端口
